...
Usage:
    python3 copy_project.py                    # Copy .py, .md files + glossary
...
MOJIBAKE_FIXES = {
    ...
    # 3-byte symbols
    "\u00e2\u0161\u00a0\u00ef\u00b8\u008f": "\u26a0\ufe0f", # warning with variant selector
    "\u00e2\u0161\u00a0": "\u26a0",           # warning sign alone
    "\u00e2\u0153\u201c": "\u2714",           # check mark
    "\u00e2\u0153\u2014": "\u2717",           # cross mark
    "\u00e2\u008f\u00b0": "\u23f0",           # alarm clock
    ...
    "\u00c2\u0090": "",                        # mangled control char DCS (U+0090)
    "\u00c2\u0091": "",                        # mangled control char (U+0091)
    "\u00c2\u0092": "",                        # mangled control char (U+0092)
    "\u00c2\u0093": "",                        # mangled control char (U+0093)
    "\u00c2\u0094": "",                        # mangled control char (U+0094)
    "\u00c2\u0095": "",                        # mangled control char (U+0095)
    ...
    "\u00c3\u201e\u00c2\u0081": "\u0101",     # a macron triple-encoded
    "\u00c3\u201e\u00c2\u00ab": "\u012b",     # i macron triple-encoded
    "\u00c3\u2026\u00c2\u008d": "\u014d",     # o macron triple-encoded
    "\u00c3\u2026\u00c2\u00ab": "\u016b",     # u macron triple-encoded
...
def copy_file_with_fixes(src, dst, dry_run=False):
    ...
    if dst_dir:
        try:
            ...
            print(f"ERROR: Failed to create directory {dst_dir}: {e}", file=sys.stderr)
            return -1
...
def find_project_files(extensions):
    ...
    try:
        ...
        print(f"ERROR: Failed to access {project_dir}: {e}", file=sys.stderr)
        return []
...
LOCAL_GLOSSARY = "/mnt/project/glossary_data.yaml"


def copy_glossary(dest_dir, dry_run=False):
    """Copy glossary_data.yaml from /mnt/project, fixing mojibake.
    ...
    if dry_run:
        print(f"  {LOCAL_GLOSSARY} -> {dest_path} [text, will fix]")
        return True

    if os.path.exists(LOCAL_GLOSSARY):
        fixes = copy_file_with_fixes(LOCAL_GLOSSARY, dest_path)
        if fixes >= 0:
            size = os.path.getsize(dest_path)
            fix_msg = f" ({fixes} fixes)" if fixes > 0 else ""
            print(f"  glossary_data.yaml ({size / 1024:.1f} KB){fix_msg}")
            return True
        else:
            print(f"ERROR: Failed to copy {LOCAL_GLOSSARY}", file=sys.stderr)
            return False
    else:
        print(f"ERROR: {LOCAL_GLOSSARY} not found", file=sys.stderr)
        return False
...
def main():
    ...
    # Copy glossary first
    if not copy_glossary(args.dest, dry_run=args.dry_run):
        return 1
        return 1
    ...
    # Exclude glossary_data.yaml - we already handled it
