...
================
    ...
    Examples:
        name_generator.py batch tsatsan:aureate:2 imperial:lower:3
        name_generator.py batch caste:mutterer:3 clone:abject:2

REGIONS AND TYPES
=================
    imperial    - Core Imperial (ya-Sattra, ya-Don)
                  personal, aureate, bureau, professional, management,
                  lower, homiletic

    tsatsan     - Ya-Tsatsa (Oracle cult city)
                  personal, aureate, family, full

    ganati      - Republic of Ganat
                  personal, unmarked, compound, scaled

    akama       - Thousand Kingdoms
                  personal, malpais, ogon, kmadhol, astrological, epithet

    avouvar     - Asovoe / ya-Tsovez
                  personal, full, precinct

    caste       - Special castes with descriptive naming
                  mutterer  - Physical descriptor (Crooked-Ear)
                  astal     - Whimsical feature (Ear Tufts)
                  draethen  - Distinguishing mark (Grey-Patch)
                  takefyeh  - Color + symbol (Blue Square)
                  pierrot   - Fragment + number (An-7)
                  choir     - Collective name (Choir at Vassel Street)

    clone       - Clone populations by status
                  personal  - Higher-status (Soh Ket)
                  batch     - Service clones (Moraveth-2)
                  abject    - Lowest-status (Semeion-9)
...
===========
    ...
    Examples in text:
        [NAME:imperial:professional:archivist] discovered the document.
        Later, [NAME:imperial:professional:archivist] told no one.
        -> Both become "Avar Kalir"
...
characters and should not be randomly generated. Currently canonical:
    ...
    Avouvar characters: Czenovae, Teszvorath, Szelvoram, Koratvel
