...
**For naming characters, see the Imperial Naming Conventions document.** That document contains generation tables, phonology guides, and regional conventions (Tsatsan, homiletic names, etc.).
...
## **The Shape of the Setting's Mysteries**

Before writing, understand what kind of mysteries this setting contains:

**The mysteries are not hidden. They are unreachable.**

The information exists. It's in documents. It's been seen. But the grammar for processing it doesn't exist, or the people who could understand it are dead, or the path from symbol to meaning has been severed. You can hold the answer in your hands and still not be able to read it.
...
Something was built. It worked. Then something broke, or changed, or was never what the builders assumed. The deep questions are forensic: *what went wrong?* Not theological: *what does God want?* Someone designed this. Someone made choices. The consequences unfolded. The mystery lives in the gap between intention and outcome.

**The question is not "what does God want?" but "why would God even want that?"**

The Testaments are available. You can read them. The Eighth Testament commanded stripping a black hole's event horizon. That's not hidden. But the Oracle precomputed the light cone. It knew the Technical Castes would fail. It knew they would be sterilized. It issued the command anyway.
...
### **Authentication Checks Form, Not Truth**

The validation system verifies grammar, not accuracy. A perfectly constructed lie authenticates; a flawed truth fails.

**In practice:** Characters can file reports that are grammatically perfect and completely misleading. Sophisticated actors exploit the gap between formal correctness and actual accuracy. The system asks "was this said properly?" not "is this true?" Show characters gaming the forms, and show other characters who don't realize the forms can be gamed.

### **All Legitimate Action Is Delegation**
...
The vocabulary does not expand. Concepts that emerged after the Interdict have no formal terms.

**In practice:** Characters route around gaps using informal speech, foreign loanwords, technical jargon, or documents that bypass authentication. The workaround reveals the constraint. A bureaucrat who needs to discuss something the vocabulary can't name will code-switch, borrow from another language, or write a "paper annex" that never touches a Lector.
...
### **The Fossil Layer Is Real But Opaque**
...
There are things that are mathematically certain that the language cannot say. There are things the language says fluently that may not be true. The gap between these is where the mysteries live.

**In practice:** When characters encounter this gap, they're not experiencing ignorance. They're experiencing tragedy. The knowledge existed. The tools existed. Something happened, and now the knowledge is inaccessible and the tools are broken. The system continues operating on assumptions it cannot verify and cannot question. Characters can see the gaps. They just can't cross them.

---

## **The Three Registers**

**Written formal (Lector-facing):** Frozen vocabulary, full particle structure, no loanwords. This is what authenticates. Officials compose it carefully, often with reference materials. It reads as archaic and stilted. No one speaks this way; it is purely for documents that must pass through the system.

**Spoken formal (educated public speech):** Particles preserved, complex constructions, but living. Loanwords appear. Circumlocutions handle gaps. This is how Aureates address each other at functions, how bureaucrats speak in meetings, how the educated discuss serious matters. It sounds formal but not dead.

**Casual (everyday):** Particles dropped, titles shortened, simplified constructions. Class position audible in how much formal structure survives. A merchant preserves more than a laborer; an Aureate speaking casually still sounds different from a commoner speaking formally.
...
**The horror isn't signaled by spooky language or cosmic imagery.** It emerges from the gap between what kind of document this is and what it contains. The reader's experience should be: "Wait. This is a \[mundane document type\]. Why does it contain \[category-violating content\]?"

When writing in-world documents:
...
The content is mechanical. The experience is mystical. The gap between them is where the setting lives.

---

# **Morphological Reference**

The following sections catalog Imperial morphology for use in generating consistent terms, partial translations, and authentication faults. This material is structured for retrieval, not study.
...
* Entries marked \[EMPEROR-SPECIFIC\] appear only in Imperial missives; **do not use in modern documents**

**What this is not:**

* A productive word-generation system (modern Imperial doesn't work that way)  
* A complete inventory (many roots are lost or unrecovered)  
* Internally consistent (natural languages aren't)
...
vel- \+ \-eth \= vel.eth (harmony-pattern, "framework of reconciliation")

vel- \+ ameth- \+ kir- \+ \-eth \= vel.ameth.kir.eth (harmony-cycle-rotation-pattern, "the calendar")

**Roots combine freely** to extend meaning. Vel.ameth.kir- is a valid stem combining three roots (harmony-cycle-rotation), but it requires a terminal to become a complete noun.
...
\[past\] vel.eth     \= "there was judging" / "judgment used to happen" (habitual)

\[past\] vel.al      \= "judgment-capacity obtained" / "one could judge" (ability)
...
Or more literally: "past / delegate-agent / contract / worker / judgment-act"
...
**Fricative elision at morpheme boundaries**: Fricatives (/Î¸/, /v/, /f/, and others) tend to elide when they occur at morpheme boundaries between vowels. The vowels on either side are still pronounced, often as a long vowel—the consonant drops but leaves a trace in vowel length.

* orath.ul → oraul (the /Î¸/ elides)  
...
## **Nominalizing Terminals**

Terminal suffixes complete stems into usable nouns. The terminal determines what kind of noun the stem becomes.

### **Abstract/Conceptual Terminals**
...
**\-gir** : Tender, nurturer. "One who cares for X." Kengir (origin-tender, farmer), varemgir (germination-tender, Bureau of the Creche). Used for caretaking rather than wielding.

### **Institutional Terminal**

**\-ara** : Institution, collective-decider, governing body. "The body that performs X." Transforms a function-root into the body that performs it. Satara (sat.ara, delegation-institution) is the council. The suffix implies collective deliberation within a bounded institutional form; it does not imply popular sovereignty (which would require shal- constructions the Lector rejects).
...
**\-est** : Authority per correct delegation. "With proper authority to X." Contraction of es-SETAL, marking that a function is exercised through proper delegation chain. Vel.est is judgment-authority. Varem.est is germination-authority. The suffix appears constantly in legal and administrative documents to distinguish the *authority* to perform a function from mere *capacity* to perform it. A surgeon has capacity to terminate a pregnancy; only one with varem.est has authority to do so.

---

## **Medial Elements**

Medial elements extend stems but cannot complete them. They must be followed by a terminal suffix.

\[No medial elements currently documented. The category exists in the morphological system but productive examples have not been identified.\]

---

## **Archaic/Fossilized Suffixes**

These appear in inherited vocabulary but are no longer productive.

**\-ai** : Archaic agentive. Appears in Morai (mor.ai). No longer productive.

**\-iq** : Appears in Tariq. Archaic orthography; the q is now pronounced as a voiced velar. Original morphological function unknown.

**\-im** : Patronymic fossil. Possibly collective or filial.

**\-gan** : Patronymic fossil. Possibly filial.

---

## **Terminal Dropping and Fossilized Forms**

### **Obvious Noun Terminals**

The suffixes **.sah** (bearer/witness) and **.en** (concrete instance) are often dropped when it is obvious that what is being spoken of is a noun. A "kessah" (friend-creature) may be called simply "kes" in casual speech. A "vor.al.en" (river, the thing of continuous flow) may appear as "voral" when the noun reading is clear from context.

This creates apparent exceptions to the "stems require terminals" rule. The terminal is present in the underlying form; it is simply not pronounced.
...
## **Emperor-Specific Vocabulary**

**CRITICAL: Do not use in modern documents.** The following terms appear only in Imperial missives. They may be archaic forms, Jargon calques, or vocabulary unique to the Emperor's perception. Tariq's analysis is speculative; the glosses below represent his best attempts, not definitions.

### **Emperor-Specific Roots**
...
* varal.eth \= the pattern of exchange (commerce? economy? something else?)
...
The Emperor's vocabulary follows Imperial morphological patterns but produces compounds that resist parsing:

**kel.sera** : Threshold \+ sera. Tariq glosses this as "crossing into visibility" or "the blooming thing," but the construction is ambiguous. Is sera modifying kel, or is kel.sera a unit?

**vel.kerith** : Judgment/reconciliation \+ kerith. What is judged? What is the kerith's role? Tariq's gloss "judgment-consumption" may be wrong.

**kerith.sah** : Kerith \+ agent/patient/witness. The thing that undergoes kerith? The thing that performs it? The thing that witnesses it?
...
**ot-** : Tool, instrument, implement. Non-technological: hand tools, simple implements. Distinct from tav- (rod, device), which carries technological connotations. Otsar (tool-wielder) means craftsman.

**oth-** : Aperture, access-hole, portal. A bounded opening. Distinct from kel-: kel- marks crossing, oth- marks the hole through which crossing occurs. The compounds oth.seth.X and oth.SETAL.ser.el appear in Archon-derived texts; their full forms and meanings are not known to modern scholarship.

### **P**

**pel-** : Coin, value, transmission. Transfer of value; extends to transfer generally. To transmit electricity or radio waves is literally to "pay." Pel.am (coin-plural, with .sah dropped) is coinage collectively. Pel.am.sar is a Bureau of the Coin official.

**phen-** : Sessile living thing, plant. Anything rooted: trees, crops, fungi. Phen.tav.en (plant-rod-thing) is a cutting. Phen.dal.eth is agriculture as institution.
...
**sat-** : Delegation, substitution, proxy-relation. All legitimate action passes through sat-. Sat.em (with .sah dropped) is the collective of delegates. Sat.ara (delegation-institution) is the council.
...
**seth-** : Becoming, instantiation, temporal-acquisition. The moment when something that was becoming *is*. Aspectually punctual where varem- is continuous. Birth is seth-; pregnancy is varem-.

**shar-** : Blade, separation, division. Physical: a sword. Abstract: splitting, dividing. Shar.don.ov (divide-distribute-act) appears in verbs of inheritance division. Shar.sar is a Bureau of the Sword official.

**shal-** : Persons as mass, people as collective entity. Distinct from shem- (blood, lineage). Shal- treats persons as an undifferentiated whole. Appears in shalath (mankind, the totality of persons). The root rarely appears outside this construction in modern usage.

**shem-** : Blood, lineage-substrate. Physical: the red fluid. Genealogical: what passes through generations (including obligation and debt). Tatharshem names the Sickly (fossilized caste name: ta.thar.shem, caste-of-disordered-blood).
...
Shav- (with the base a-vowel) means directional application, partiality, treating different parties or directions differently. A shav.vel.ov (directional-judgment-act) is a ruling that explicitly favors some parties. Shav.eth (directionality-pattern) describes a policy designed to produce different outcomes for different subjects.

The roots appear frequently in legal and administrative contexts. An official accused of shav.ov (directional-action, favoritism) might defend themselves by demonstrating shev.eth (uniform-pattern) in their decisions. The distinction between legitimate shav- (a tax policy that intentionally favors agriculture over mining) and illegitimate shav- (a judge who favors his relatives) is a matter of whether the directionality was properly delegated.
...
**thev-** : Axis, eigenvector, principal direction. Technical Caste terminology that entered ordinary language for cardinal directions. Thev.rem (axis-aligned) means north. Thev.nem (axis-opposed) means south.
...
**themal-** : Scales, weights, balances; also discernment, choosing between options. The Bureau of the Scale uses this root. Themal.ul contracts to temuul (the property of discernment).
...
**yev-** : Building, structure, facility. Contracted from ya.kev (place of enclosure). Yev.ten.en contracts to yevten (warehouse). Yev.e.mar is a superintendent of a facility. \[Contraction\]

---

## **Class VII Nouns**

Class VII is a grammatical category containing a small number of nouns with unusual properties. The class includes the Emperor (SETAL), the Oracles, and the Record.

**Atemporality**: Class VII nouns resist temporal modification. One cannot say "the current Emperor" or "the former Oracle" or "the Record as of last year." They exist outside sequence. Verbs attached to Class VII nouns default to atemporal readings: disclosure, command, eternal obtaining.

**Uncountability**: Class VII nouns resist counting and sequencing. One cannot say "the first Emperor" or "the third Oracle" in formal register. (Informal speech manages this through circumlocution: "the Oracle whose Testament addressed the Plague.") The grammar does not admit that there could be more than one, or that the current instance is one of a series.
...
* Continuous-aspect perception verbs ("is looking")  
* Past-tense experiential verbs ("held your hand")  
...
**Compounds with Class VII nouns**: When SETAL appears in compounds, it occupies the authority-source position: SETAL.ser (law), SETAL.keth (Emperor-voice), SETAL.vel (Emperor-judgment), SETAL.ul (Dominion). These compounds authenticate; most others involving SETAL do not.

The practical effect is that formal Imperial cannot express succession, incapacity, institutional change, or the passage of time as it affects the highest authorities. These topics are handled through informal speech, circumlocution, or unwritten understanding.
...
**SETAL.vel** : Emperor-judgment. The evaluation function of supreme authority. May carry the mathematical sense of vel- (reconciliation of frameworks) rather than mere legal adjudication.

**SETAL.ul** : Dominion. The quality of being Emperor. The Virtue borne by the Second Oracle.
...
## **Unanalyzable Vocabulary**

Some vocabulary does not decompose into Imperial roots. This includes:

* **Non-Imperial names** from absorbed or predating languages  
* **Detached suffix names** where the original root was lost  
* **Tsatsan syllables** derived from Jargon notation, bearing no relationship to Imperial morphology

See the Imperial Naming Conventions document for details on these categories.
...
### **Archaic Forms with Inaccessible Meaning**

Some terms appear in ancient documents but resist modern interpretation:

* **vel.om**: Frequent in pre-Interdict legal texts. The obvious reading is "judgment-surface" (courtroom), but Technical Caste documents also use vel.om as an apparent synonym for mir.themal.om (witness-discernment-surface). Whatever this concept was, it required two different Imperial calques, neither adequate.  
* **oth.seth**, **oth.SETAL.ser**: Appear in Archon-derived texts and First Whorl documents. Context suggests perceptual or access functions. If these were discrete approximations of Jargon coordinates describing the Emperor's sensorium, the semantic content they approximated cannot be recovered.

### **Politically Blocked Forms**

Some compounds do not authenticate for reasons that are political rather than grammatical. The Lector specifications that block these forms are part of the Interdict-era freeze.

**Authority-origin constructions:**

* **vel.ken** (origin-of-judgment): Parses correctly but requires a clause specifying what the judgment originates from. Without specification, presumes ultimate origin from SETAL.  
* **ken.vel** (judgment-as-origin): Rejected. Judgment cannot occupy the origin position.  
* **shal.vel.ath** (exclusively-popular-judgment): Does not authenticate. Mankind is what the Emperor rules; mankind cannot be what authority derives from.

**Governance without delegation:**
...
The effect is that democratic or collective-action concepts require circumlocution, loanwords, or constructions that frame the collective as acting *under* authority rather than *as* authority.
...
* **Succession constructions**: SETAL rejects temporal modifiers and counting. No construction for "when the current Emperor dies..."  
* **Incapacity constructions**: "The Emperor is unable to govern" requires a subject the grammar rejects.  
* **Error constructions**: "\[SETAL\] produces errors" requires SETAL, which presumes what follows is disclosure. Disclosure is true by virtue of being disclosed.
...
**The problem:** You need readers to hear social position without characters announcing it.

**The tools:**

* Particle preservation vs. dropping (formal: "traveling to ya-Don"; casual: "going to Don")  
* Vocabulary complexity (educated speakers use longer constructions; laborers compress)  
* Register-switching within conversations (an Aureate relaxing formality is different from a commoner being formal)

**What to show:**

* A merchant who preserves more formal structure than a dockworker  
* An Aureate whose casual speech is still more elaborate than a commoner's formal speech  
* A provincial who overcorrects, using hyper-formal constructions that mark them as trying too hard  
* A sophisticated urbanite who knows exactly when to drop particles and when that would be an insult

**What not to explain:** Don't tell readers "she spoke formally." Render the speech. Let them feel the difference.
...
* Written formal register: frozen vocabulary, full particles, no loanwords  
* Authentication constraints: Lectors verify form, not truth  
* The gap between what authenticates and what is actually true  
* Paper annexes and informal channels that bypass authentication

**What to show:**

* Documents that are grammatically perfect and completely misleading  
* The careful composition process (officials with reference materials, checking constructions)  
...
**The problem:** A character encounters something formal Imperial cannot express.

**What actually happens:**

* The sentence doesn't come together. They start to speak and trail off.  
* They use the wrong construction and it sounds strange to their own ears.  
* They file a report that carefully routes around what they actually saw.  
* They code-switch to informal speech, loanwords, or circumlocution.

**What doesn't happen:**

* The grammar doesn't "forbid" them. There's no wall.  
* They don't consciously think "I cannot say this." The prohibition operates before language.  
* They don't rage against the system productively. The system is too large to have a face.
...
**CRITICAL: This section describes the Emperor's usage. Do not use these constructions in modern documents.**

**The problem:** You're writing correspondence from the Emperor, or characters discussing/noticing his usage.

**What the Emperor does:**

* Uses the indirect-reflexive with perception verbs ("is looking" rather than "looks")  
* Uses past-tense experiential verbs with the indirect-reflexive ("held your hand")  
* Uses modal verbs of unfulfilled desire ("wishes")  
* Uses negated cognitive verbs ("does not know," "does not understand")  
* Uses vocabulary that appears nowhere else in the corpus (see below)

These are constructions a child would use before learning correct forms. The grammar was supposedly engineered around his nature; he violates it consistently. He writes as though observing himself from outside, uncertain of his own mental states.

**The Emperor also uses vocabulary that appears nowhere else in the corpus:**

* sera- (related to ser-, distinction unclear)  
* kerith (a process Tariq cannot fully characterize)  
...
* Report it. SETAL presumes what follows is disclosure or command. Disclosure is true by virtue of being disclosed.  
* Discuss it formally. There is no construction for "the source of all correct disclosure produces incorrect disclosure."  
* Ignore it. The wrongness sits in routine correspondence, being wrong.
...
**The problem:** Characters encounter Jargon-derived terms or concepts that are mathematically certain but that Imperial cannot hold.
...
* A term in a technical manual that points at something the reader's mind can't hold  
* The gap between having the answer and having the question it was answering  
* Characters who can see the gaps but cannot cross them  
* The ascending structure: each revelation opens a larger question

**What not to show:**

* Characters seeking the mystical (it finds them through paperwork)  
* Ultimate answers (the structure continues beyond what the text provides)  
* The cosmic as separate from the bureaucratic (they use the same machinery)

## **Generating Consistent Imperial Terminology**

**When you need this:** Creating technical terms, institutional vocabulary, or compounds that should feel Imperial.
...
**Example:** You need a term for "electrical current."

* tsir- (spark, charge) \+ vor- (flow) \+ \-al (continuous)  
* Result: tsir.vor.al (spark-flow-continuous)

**Example:** You need a term for "the calendar."

* ameth- (cycle) \+ kir- (rotation, seasonal) \+ \-eth (pattern)  
* Result: ameth.kir.eth (cycle-rotation-pattern)
...
* Make class audible without stating it  
* Let characters code-switch between registers based on context  
* Show workarounds: loanwords, circumlocutions, informal channels  
* Render the speech; don't explain it

**For documents:**

* Use frozen vocabulary and full particle structure for Lector-facing text  
* Include category violations in mundane documents  
* Show the gap between formal correctness and actual truth  
* Show the composition process and the workarounds

**For authentication failures:**

* Write characters failing to form the thought, not hitting a wall  
* Show sentences that don't come together, constructions that sound wrong  
* Let the prohibition be invisible because it operates before language

**For the Emperor's anomalies:**

* Use the indirect-reflexive with perception, experience, desire, and negated cognition  
* Use Emperor-specific vocabulary (sera-, kerith, varal, \-veth)  
* Create constructions with unclear morpheme boundaries (kel.sera.varem.ov.el)  
* Let the wrongness sit in routine correspondence without comment  
* Show characters who know and cannot report  
* **Do not use Emperor-specific constructions in modern documents**
...
* Use the morphological reference to build consistent terms  
* Apply phonological processes appropriately  
* Remember the Interdict freeze: no formal vocabulary for new concepts  
* Remember that stems require terminal suffixes to become complete words
...
* Don't give characters access to the complete picture  
* Don't provide ultimate answers  
* Don't let the cosmic feel separate from the bureaucratic  
* Don't reach the bottom of the mystery structure

**For terminology:**

* Don't generate formal terms for post-Interdict concepts  
* Don't assume generated compounds would authenticate  
* Don't have ordinary speakers know their words' etymologies  
* Don't leave stems without terminal suffixes
