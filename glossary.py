...
def load_yaml_safe(filepath: str) -> dict:
    """Load YAML file with mojibake fixing."""
...
def load_glossary(yaml_path: Path = None) -> Dict[str, GlossaryEntry]:
    ...
    # Use mojibake-safe loader
    data = load_yaml_safe(str(yaml_path))
...
def search(query: str, fuzzy: bool = True) -> List[GlossaryEntry]:
    """Search terms, shorts, and details for query string.
    
    If no exact matches found and fuzzy=True, falls back to fuzzy term matching.
    """
    ...
    # Fuzzy fallback on term names if no results
    if not results and fuzzy:
        all_terms = [e.term for e in ENTRIES.values()]
        similar = get_close_matches(query, all_terms, n=5, cutoff=0.5)
        for term_name in similar:
            entry = lookup(term_name)
            if entry:
                results.append(entry)
...
def get_inherited_language(entry: GlossaryEntry, max_depth: int = 10) -> tuple[str, str]:
    """Get language for a location, inheriting from parent if not set locally.
    
    Returns:
        (language_text, source) where source is the name of the location 
        where the language was defined, or empty strings if none found.
    """
    if not entry or not entry.location_features:
        return ("", "")
    
    # Check if this entry has its own language
    if entry.location_features.language:
        return (entry.location_features.language, entry.term)
    
    # Walk up parent chain
    current = entry
    depth = 0
    while depth < max_depth:
        parent = get_parent(current.term)
        if not parent:
            break
        if parent.location_features and parent.location_features.language:
            return (parent.location_features.language, parent.term)
        current = parent
        depth += 1
...
def format_entry(entry: GlossaryEntry, verbose: bool = False) -> str:
    ...
    if entry.location_features:
        ...
        # Show language (with inheritance)
        lang_text, lang_source = get_inherited_language(entry)
        if lang_text:
            if lang_source == entry.term:
                lines.append(f"  Language: {lang_text}")
            else:
                lines.append(f"  Language: {lang_text} (from {lang_source})")
        if lf.children:
